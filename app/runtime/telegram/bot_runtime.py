...
def build_telegram_runtime(settings: TelegramBotSettings) -> tuple[Bot, Dispatcher]:
    ...
    dispatcher.include_router(
        build_identity_review_router(
            TelegramAdminCustomerIdentityHandler(
                __import__(
                    "app.application.customer_identity",
                    fromlist=["CustomerIdentityService"],
                ).CustomerIdentityService(
                    __import__(
                        "app.infrastructure.persistence.customer_identity_repository",
                        fromlist=["SupabaseCustomerIdentityRepository"],
                    ).SupabaseCustomerIdentityRepository(client)
                )
            )
        )
    )
