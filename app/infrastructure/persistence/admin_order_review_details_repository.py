...
class SupabaseAdminOrderReviewDetailsRepository(AdminOrderReviewDetailsRepository):
    ...
    async def get_details(
        ...
        try:
            ...
            latest_receipt = None
            if row.get("receipt_submission_id") is not None:
                if submitted_at is None:
                    raise ValueError("receipt timestamp is required")
                latest_receipt = AdminReceiptView(
                    submission_id=UUID(str(row["receipt_submission_id"])),
                    attempt_number=int(row["receipt_attempt_number"]),
                    input_type=str(row["receipt_input_type"]),
                    processing_status=str(row["receipt_processing_status"]),
                    linkage_status=str(row["receipt_linkage_status"]),
                    telegram_file_id=str(row["receipt_telegram_file_id"]) if row.get("receipt_telegram_file_id") else None,
                    mime_type=str(row["receipt_mime_type"]) if row.get("receipt_mime_type") else None,
