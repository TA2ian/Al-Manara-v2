-- An atomically created purchase order is immediately payable.
-- Keeping DRAFT as a persisted runtime state would leave the customer order
-- invisible to the pending-payment queue after successful creation.
