-- A submitted customer receipt is accepted for human review without being
-- treated as financially verified. This keeps approval strictly manual.
alter table receipt_submissions
    drop constraint if exists receipt_processing_status_valid;

alter table receipt_submissions
    add constraint receipt_processing_status_valid
    check (processing_status in ('PENDING','PROCESSING','SUBMITTED','SUCCEEDED','FAILED','ESCALATED'));
...
begin
    ...
    if p_processing_status in ('SUBMITTED','SUCCEEDED') then
        ...
        if v_current_status <> 'PENDING_PAYMENT' then
            raise exception 'receipt submission requires order in PENDING_PAYMENT';
        end if;

        update orders as o
           set status = 'PAYMENT_SUBMITTED', version = o.version + 1, updated_at = now()
         where o.internal_order_id = v_order_id and o.version = v_order_version;
        ...
        insert into audit_logs (
            ...
            null, null, 'order.receipt_submitted', 'order', v_order_id::text,
        ...
        update orders as o
         ...
         where o.internal_order_id = v_order_id and o.version = v_order_version + 1;
        ...
        insert into audit_logs (
            ...
            null, null, 'order.receipt_queued_for_review', 'order', v_order_id::text,
            jsonb_build_object('status', 'PAYMENT_SUBMITTED', 'version', v_order_version + 1),
    ...
    return query
    select r.id, r.internal_order_id, r.attempt_number, r.input_type,
           r.transaction_reference, r.telegram_file_id, r.mime_type,
           r.submitted_at, r.processing_status, r.linkage_status, r.failure_reason
      from receipt_submissions r where r.id = p_submission_id;
end;
$$;
