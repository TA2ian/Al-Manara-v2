-- Complete the six-network configuration and fee-aware order creation
-- after 0056 has committed the POLYGON enum label.
...
begin
    ...
    select nc.code, nc.display_name, nc.enabled, nc.address_regex,
           ...
           nc.network_fee_amount, nc.config_version
...
begin
    ...
    select ik.response_json into v_existing
     ...
     for update;
    if found then
        return query select (v_existing ->> 'internal_order_id')::uuid,
    ...
    select u.id into v_user_id from users u
     where u.telegram_user_id = p_user_id and not u.is_disabled;
    ...
    select w.user_id, w.network_code, w.status, w.address
      ...
      from wallets w where w.id = p_wallet_id for share;
    ...
    select nc.enabled, nc.min_amount, nc.max_amount,
      ...
      from network_configs nc where nc.code = p_network_code::network_code for share;
    ...
    select u.verified_name, u.verified_shamcash_account, u.payment_identity_verified_at
      ...
      from users u where u.id = v_user_id for share;
    if v_identity_name is null or v_identity_account is null or v_identity_verified_at is null then raise exception 'customer payment identity is not verified'; end if;
    if btrim(v_identity_name) <> btrim(p_customer_verified_name_snapshot)
       or btrim(v_identity_account) <> btrim(p_customer_shamcash_account_snapshot) then raise exception 'customer identity snapshot mismatch'; end if;

    select pm.id into v_payment_method_id from payment_methods pm
     where pm.code = 'SHAM_CASH' and pm.status = 'ENABLED';
    ...
    insert into order_financial_snapshots (
        internal_order_id, requested_amount, fee_percent, fee_amount, network_fee_amount,
        net_usdt_amount, payment_currency, exchange_rate, local_amount,
        rounding_policy_version, network_config_version
    ) values (
        p_internal_order_id, p_requested_amount, p_fee_percent, p_fee_amount, p_network_fee_amount,
        p_net_usdt_amount, p_payment_currency::currency_code, p_exchange_rate, p_local_amount,
        p_rounding_policy_version, v_network_config_version
    );

    insert into audit_logs (actor_telegram_user_id, action, target_type, target_id, new_value, metadata)
    values (
    ...
    values (
        ...
        jsonb_build_object('internal_order_id', p_internal_order_id,
...
exception
    when unique_violation then
        if exists (select 1 from orders o where o.public_order_code = p_public_order_code) then raise exception 'public order code collision'; end if;
        if exists (select 1 from idempotency_keys ik where ik.telegram_user_id = p_user_id and ik.operation = p_operation and ik.idempotency_key = p_idempotency_key) then raise exception 'idempotency key collision'; end if;
        raise;
end;
$$;
