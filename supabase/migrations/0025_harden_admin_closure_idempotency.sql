-- Harden administrative closure replay: authorization must happen before
-- reading an existing idempotency result. The closure key is admin-scoped,
-- but disabled/revoked administrators must not receive privileged replay data.
...
begin
    ...
    -- Authorize before any idempotency replay.
