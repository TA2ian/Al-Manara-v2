-- Fix the completion idempotency race by serializing on the order
-- before consuming the one-time admin confirmation.
--
-- A concurrent same-key request must be allowed to wait for the first
-- completion and then replay its committed idempotency result. It must not
-- consume/fail on the shared confirmation before it can observe that result.
--
-- No blockchain transaction is submitted here; fulfillment remains manual.
...
begin
    ...
    -- Serialize concurrent completion attempts on the order before consuming
    -- the one-time confirmation. This lets a waiting same-key replay observe
    -- the committed idempotency row instead of losing its confirmation race.
    ...
    -- Re-check idempotency after acquiring the order lock. A concurrent
    -- completion may have committed the result while this transaction waited.
    select f.result into v_existing
      from order_fulfillment_idempotency f
     where f.idempotency_key = btrim(p_idempotency_key)
       and f.operation = 'complete'
    ...
    -- Confirmation is consumed only after the operation has won the order
    -- serialization point and no committed idempotency result exists.
