-- Privileged order transitions must not trust actor_type supplied by the caller.
-- Customer/system transitions remain possible only when no admin actor is supplied.
...
begin
    ...
    if p_actor_telegram_user_id is not null then
