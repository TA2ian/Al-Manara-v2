...
select ok(not has_function_privilege('anon', 'public.create_purchase_order_atomic(uuid,text,bigint,uuid,text,text,numeric,numeric,numeric,numeric,text,numeric,numeric,text,text,text,text,text,timestamptz,timestamptz,text,text)', 'execute'), 'atomic order creation RPC is not publicly executable');
